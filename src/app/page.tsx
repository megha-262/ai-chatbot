...
export default function Home() {
  ...
  const handleVoiceChat = () => {
    // Navigate to chat page with voice mode parameter
  ...
  const handleLearnCPR = () => {
    // Open CPR guide in new tab
    window.open('https://www.redcross.org/take-a-class/cpr/performing-cpr/cpr-steps', '_blank');
  };

  const handleCheckSymptoms = () => {
    // Navigate to chat with emergency symptom check
    router.push('/chat?mode=emergency');
  };

  const handleFindEmergencyServices = () => {
    // Get user location and find nearby emergency services
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          const { latitude, longitude } = position.coords;
          // Open Google Maps with emergency services search
          window.open(`https://www.google.com/maps/search/emergency+services/@${latitude},${longitude},15z`, '_blank');
        },
        (_error) => {
          // Fallback to general emergency services search
          window.open('https://www.google.com/maps/search/emergency+services', '_blank');
        }
      );
    } else {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link href="/chat" className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-4 rounded-lg text-lg font-semibold transition-colors shadow-lg text-center">
                Start Health Chat
              </Link>
              <Link href="/emergency" className="bg-red-600 hover:bg-red-700 text-white px-8 py-4 rounded-lg text-lg font-semibold transition-colors shadow-lg text-center">
                🚨 Emergency Help
              </Link>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-lg border-l-4 border-yellow-500">
              ...
              <button 
                ...
                className="w-full bg-yellow-600 hover:bg-yellow-700 text-white py-2 rounded-lg font-semibold transition-colors"
              >
                ⚠️ Check Symptoms
              </button>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-white dark:bg-gray-700 rounded-2xl p-8 shadow-xl">
            ...
            <div className="mt-8 text-center">
              <button 
                onClick={handleFindEmergencyServices}
